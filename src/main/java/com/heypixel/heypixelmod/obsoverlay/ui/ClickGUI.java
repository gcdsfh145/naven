...
public class ClickGUI extends Screen {
   ...
   IntValue hoveringIntValue;
   IntValue draggingIntValue;
   ...
   public ClickGUI() {
      super(Component.nullToEmpty("ShaoYu"));
      for (Value value : Naven.getInstance().getValueManager().getValues()) {
         if (!this.valuesAnimation.containsKey(value)) {
            this.valuesAnimation.put(value, new SmoothAnimationTimer(0.0F));
         }
      }
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
      try {
         ...
         if (this.bindingModule == null || mouseButton != 3 && mouseButton != 4) {
            if (mouseButton != 2 && this.bindingModule == null) {
               ...
               if (mouseButton == 0) {
                  ...
                  if (this.hoveringIntValue != null) {
                     this.draggingIntValue = this.hoveringIntValue;
                  }
         ...
         System.err.println("Error in mouseClicked: " + e.getMessage());
         e.printStackTrace();
         return false;
      }
   }
   ...
   public boolean mouseReleased(double pMouseX, double pMouseY, int pButton) {
      try {
         ...
         if (this.draggingIntValue != null) {
            this.draggingIntValue = null;
         }
   ...
   public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
      try {
         ...
         System.err.println("Error in keyPressed: " + e.getMessage());
         e.printStackTrace();
         return false;
      }
   }
   ...
   protected void init() {
      try {
         ...
         this.valuesAnimation.forEach((value, animation) -> {
            try {
               if (value.getValueType() == ValueType.MODE || value.getValueType() == ValueType.INT) {
                  animation.value = 0.0F;
                  animation.target = 255.0F;
               }
            } catch (Exception e) {
               System.err.println("Error initializing value animation: " + e.getMessage());
               e.printStackTrace();
            }
         });
      } catch (Exception e) {
         System.err.println("Error initializing ClickGUI: " + e.getMessage());
         e.printStackTrace();
      }
   }
   ...
   public boolean mouseScrolled(double pMouseX, double pMouseY, double pDelta) {
      try {
         ...
         System.err.println("Error in mouseScrolled: " + e.getMessage());
         e.printStackTrace();
         return false;
      }
   }
   ...
   public void onShader(EventShader e) {
      try {
         ...
         System.err.println("Error in onShader: " + ex.getMessage());
         ex.printStackTrace();
      }
   }
   ...
   public void render(GuiGraphics g, int mouseX, int mouseY, float pPartialTick) {
      try {
         ...
         opensans.render(stack, "ShaoYu", (double)(windowX + 50.0F - opensans.getWidth("ShaoYu", 0.75) / 2.0F), (double)(windowY + 5.0F), Color.WHITE, true, 0.75);
         opensans.setAlpha(1.0F);
         if (this.selectedCategory != null) {
         ...
         if (this.renderValues != null) {
            ...
            // 渲染 IntValue
            this.hoveringIntValue = null;
            for (Value valuex : this.renderValues) {
               if (valuex.isVisible() && valuex.getValueType() == ValueType.INT) {
                  try {
                     IntValue intValue = valuex.getIntValue();
                     SmoothAnimationTimer animationx = this.valuesAnimation.get(intValue);
                     if (animationx == null) {
                        animationx = new SmoothAnimationTimer(0.0F);
                        this.valuesAnimation.put(intValue, animationx);
                     }

                     if (isValueInBound
                             && RenderUtils.isHoveringBound(mouseX, mouseY, windowX + 140.0F, windowY + valueHeight + motion + 39.5F, windowWidth - 155.0F, 10.0F)) {
                        this.hoveringIntValue = intValue;
                     }

                     opensans.render(stack, valuex.getName(), (double)(windowX + 140.0F), (double)(windowY + valueHeight + motion + 25.0F), Color.WHITE, true, 0.4);
                     String currentValue = intValue.getCurrentValue() + " / " + intValue.getMaxValue();
                     opensans.render(
                             stack,
                             currentValue,
                             (double)(windowX + windowWidth - opensans.getWidth(currentValue, 0.4) - 15.0F),
                     ...
                     float stage = (float)(intValue.getCurrentValue() - intValue.getMinValue()) / (float)(intValue.getMaxValue() - intValue.getMinValue());
                     int enabledColor = Colors.getColor(54, 98, 236, 255);
                     RenderUtils.drawRoundedRect(
                             stack, windowX + 140.0F, windowY + valueHeight + motion + 42.0F, windowWidth - 155.0F, 5.0F, 3.0F, Colors.getColor(0, 0, 0, 150)
                     );
                     ...
                     System.err.println("Error rendering IntValue: " + e.getMessage());
                     e.printStackTrace();
                  }
               }
            }
         ...
         if (this.draggingIntValue != null) {
            try {
               float stage = ((float)mouseX - windowX - 140.0F) / (windowWidth - 160.0F);
               int value = this.draggingIntValue.getMinValue() + (int)((this.draggingIntValue.getMaxValue() - this.draggingIntValue.getMinValue()) * stage);

               if (value < this.draggingIntValue.getMinValue()) {
                  value = this.draggingIntValue.getMinValue();
               }
               if (value > this.draggingIntValue.getMaxValue()) {
                  value = this.draggingIntValue.getMaxValue();
               }

               // 应用步进值
               if (this.draggingIntValue.getStep() > 0) {
                  value = (value / this.draggingIntValue.getStep()) * this.draggingIntValue.getStep();
               }

               this.draggingIntValue.setCurrentValue(value);
            } catch (Exception e) {
               System.err.println("Error dragging IntValue: " + e.getMessage());
               e.printStackTrace();
               this.draggingIntValue = null;
            }
         }
